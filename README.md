...
Three important file. watch file, role config file, crd file.
...
we will create a new namespace/project called operator-helloworld. Then We will then start up our Operator and ensure our operator only watches for Custom Resources within this namespace.
...
use the operator-sdk CLI to create an Ansible Operator project with the 'init' and create commands.
...
Initialize a new Ansible-based Operator SDK project for the helloworld Operator:

```$ operator-sdk init --plugins=ansible --domain=hello.example.com```

Among the files generated by this command is a Kubebuilder PROJECT file. Subsequent operator-sdk commands (and help text) run from the project root read these file and are aware that the project type is Ansible.
...
Create the helloworld API and generate an Ansible role skeleton. API is endpoint of helloworld resource.

This will ensure our controller is setup to watch the hello resource with APIVersion 'cache.example.com/v1alpha1' and Kind 'Memcached'.
